# Impressum {-}

*Folding in R: Origami, Manifolds, and Dimension Reduction*

R. Heller\
ORCID: [0000-0001-8006-9742](https://orcid.org/0000-0001-8006-9742)

Source repository: <https://github.com/r-heller/folding-in-r>\
Online edition: <https://r-heller.github.io/folding-in-r/>

Prose is licensed under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/).
Code is licensed under the MIT License; see `LICENSE-CODE.md` in the repository.

This is a living document. The version you are reading was rendered on
`r Sys.Date()`.
